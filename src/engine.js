...
export const RIDER_SKILL_LABELS = {
  speed:'Speed', acceleration:'Acceleration', power:'Power', climbing:'Climbing', endurance:'Endurance',
  recovery:'Recovery', control:'Bike control', strategy:'Strategy', mentality:'Mental strength'
};
export const RIDER_SKILL_KEYS = Object.keys(RIDER_SKILL_LABELS);

// A rider's rarity fixes permanent base talent. The career multiplier creates the
// season ceiling, then the stage specialty determines where that ceiling applies.
// Specialists retain 100% of the season ceiling in their core skills and 90% in
// non-core skills. All-rounders receive 95% everywhere: versatile, but never the
// best pure specialist simply because they have no weakness.
export const SPECIALTY_CORE_SKILLS = {
  climber: ['climbing','endurance','recovery','mentality'],
  sprinter: ['speed','acceleration','power','strategy'],
  puncheur: ['acceleration','power','climbing','strategy'],
  rouleur: ['power','endurance','control','strategy'],
  'time-trialist': ['power','endurance','strategy','control','mentality'],
  cobbles: ['control','power','mentality','strategy','endurance'],
  'all-rounder': [...RIDER_SKILL_KEYS]
};
const SPECIALIST_SECONDARY_FACTOR = .90;
const ALL_ROUNDER_FACTOR = .95;

// Stage results are calculated from the skills that matter for that terrain.
const STAGE_SKILL_WEIGHTS = {
  flat:         {speed:.30,acceleration:.22,power:.15,strategy:.10,control:.08,endurance:.08,mentality:.04,recovery:.03},
  hilly:        {power:.20,climbing:.18,acceleration:.16,endurance:.14,strategy:.12,mentality:.08,control:.07,recovery:.05},
  puncheur:     {acceleration:.24,power:.22,climbing:.14,strategy:.12,mentality:.10,endurance:.08,control:.06,speed:.04},
  mountain:     {climbing:.30,endurance:.20,recovery:.15,mentality:.12,strategy:.10,power:.07,control:.04,acceleration:.02},
  'time-trial': {power:.24,endurance:.20,strategy:.17,mentality:.14,control:.10,recovery:.08,speed:.05,acceleration:.02},
  cobbles:      {control:.24,power:.22,mentality:.16,strategy:.12,endurance:.10,acceleration:.08,speed:.06,recovery:.02}
};
...
function freshStats(){
  return {
    ...
    bestResults:[],grandTourResults:[],raceWinDetails:[],stageWinDetails:[],jerseyWinDetails:[],teamAwardDetails:[],uciPointDetails:[]
  };
}
...
function normalizeStats(stats={}){
  ...
  normalized.bestResults=stats.bestResults||[];normalized.grandTourResults=stats.grandTourResults||[];normalized.raceWinDetails=stats.raceWinDetails||[];normalized.stageWinDetails=stats.stageWinDetails||[];normalized.jerseyWinDetails=stats.jerseyWinDetails||[];normalized.teamAwardDetails=stats.teamAwardDetails||[];normalized.uciPointDetails=stats.uciPointDetails||[];
  return normalized;
}
...
function countryCyclingProfile(code){return COUNTRY_CYCLING_PROFILES[code]||COUNTRY_CYCLING_PROFILES.default;}
function chooseNationality(random,rarity='common'){
  const eliteFactor=['generational','legend','epic'].includes(rarity)?1:0;
  return weightedPick(random,Object.entries(COUNTRY_DATABASE).map(([value,data])=>{const profile=countryCyclingProfile(value),tradition=profile.tradition||.75,elite=profile.elite||tradition;const multiplier=eliteFactor?Math.pow(elite,rarity==='generational'?1.45:rarity==='legend'?1.25:1.08):Math.pow(tradition,.45);return{value,weight:data.weight*multiplier};}));
}
function terrainForCountry(random,code){const weights=countryCyclingProfile(code).terrain||COUNTRY_CYCLING_PROFILES.default.terrain;return weightedPick(random,TERRAIN_TYPES.map(value=>({value,weight:weights[value]||1})));}
...
function careerFactorFor(rider,year){
  const elapsed=Math.max(0,year-(rider.debutYear||year)),progress=clamp(elapsed/Math.max(1,(rider.careerLength||10)-1),0,1),debutAge=rider.debutAge??Math.max(18,(rider.age||20)-elapsed),age=debutAge+elapsed;
...
function annualShapeFor(rider,year){const random=mulberry32(hashString(`${rider.id}|${year}|shape`));return .97+random()*.06;}
export function careerMultiplierForYear(rider,year){return clamp(careerFactorFor(rider,year)*annualShapeFor(rider,year),.70,1.02);}
function skillAverage(skills){const values=RIDER_SKILL_KEYS.map(key=>Number(skills?.[key])||0);return values.reduce((sum,value)=>sum+value,0)/Math.max(1,values.length);}
function geneticSkillNoise(rider){const random=mulberry32(hashString(`${rider.id}|skill-signature-v15`));return Object.fromEntries(RIDER_SKILL_KEYS.map(key=>[key,random()*6-3]));}
function rebalanceSkillGroup(raw,keys,targetMean){if(!keys.length)return{};const desired=Math.round(targetMean*keys.length),out=Object.fromEntries(keys.map(key=>[key,clamp(Math.round(raw[key]),30,100)]));let delta=desired-keys.reduce((sum,key)=>sum+out[key],0),guard=0;while(delta!==0&&guard<3000){const direction=delta>0?1:-1,candidates=keys.filter(key=>direction>0?out[key]<100:out[key]>30).sort((a,b)=>direction>0?(raw[b]-out[b])-(raw[a]-out[a]):(out[b]-raw[b])-(out[a]-raw[a]));if(!candidates.length)break;out[candidates[guard%candidates.length]]+=direction;delta-=direction;guard++;}return out;}
export function buildRiderSkills(rider,year){
  const base=clamp(rider.baseSkill??rider.potential??70,50,100),annualMultiplier=careerMultiplierForYear(rider,year),seasonCeiling=clamp(base*annualMultiplier,35,100),terrain=rider.terrain||'all-rounder',core=SPECIALTY_CORE_SKILLS[terrain]||SPECIALTY_CORE_SKILLS['all-rounder'],coreSet=new Set(core),noise=geneticSkillNoise(rider);
  if(terrain==='all-rounder'){
    const target=clamp(seasonCeiling*ALL_ROUNDER_FACTOR,30,100),raw=Object.fromEntries(RIDER_SKILL_KEYS.map(key=>[key,target+noise[key]])),skills=rebalanceSkillGroup(raw,RIDER_SKILL_KEYS,target);
    return{skills,annualMultiplier,seasonCeiling,annualRating:skillAverage(skills)};
  }
  const secondary=RIDER_SKILL_KEYS.filter(key=>!coreSet.has(key)),coreTarget=clamp(seasonCeiling,30,100),secondaryTarget=clamp(seasonCeiling*SPECIALIST_SECONDARY_FACTOR,30,100),raw=Object.fromEntries(RIDER_SKILL_KEYS.map(key=>[key,(coreSet.has(key)?coreTarget:secondaryTarget)+noise[key]])),skills={...rebalanceSkillGroup(raw,core,coreTarget),...rebalanceSkillGroup(raw,secondary,secondaryTarget)};
  return{skills,annualMultiplier,seasonCeiling,annualRating:skillAverage(skills)};
}
export function riderSkillAverage(rider){if(rider.skills)return skillAverage(rider.skills);const seasonal=clamp((rider.baseSkill??rider.potential??70)*(rider.annualMultiplier??rider.careerFactor??.9),35,100);if(rider.terrain==='all-rounder')return seasonal*ALL_ROUNDER_FACTOR;const core=(SPECIALTY_CORE_SKILLS[rider.terrain]||[]).length;return seasonal*((core+(RIDER_SKILL_KEYS.length-core)*SPECIALIST_SECONDARY_FACTOR)/RIDER_SKILL_KEYS.length);}
export function stageSkillRating(rider,profile){const weights=STAGE_SKILL_WEIGHTS[profile]||STAGE_SKILL_WEIGHTS.hilly,skills=rider.skills||buildRiderSkills(rider,rider.skillYear||rider.debutYear||2026).skills;return Object.entries(weights).reduce((sum,[key,weight])=>sum+(skills[key]||riderSkillAverage(rider))*weight,0);}
function grandTourGcRating(rider){const s=rider.skills||{},avg=riderSkillAverage(rider);return (s.climbing??avg)*.24+(s.endurance??avg)*.20+(s.recovery??avg)*.16+(s.strategy??avg)*.14+(s.mentality??avg)*.12+(s.power??avg)*.08+(s.control??avg)*.06;}
function makeRider(random,{teamId,tier,usedNames,ageRange=null,year=2026,forcedRarity=null}){
  const rarity=forcedRarity||rarityForRandom(random,tier,false),range=RARITIES[rarity],nationality=chooseNationality(random,rarity),name=nameForCountry(random,nationality,usedNames);
  const potential=randInt(random,range.min,range.max);
  ...
  const terrain=terrainForCountry(random,nationality),program=pick(random,PROGRAM_TYPES),profile=developmentProfile(random),debutAge=tier==='u23'?18:randInt(random,18,Math.min(22,age)),careerLength=Math.max(10,retirementAge-debutAge),debutYear=year-(age-debutAge);
  const rider={
    ...
    developmentProfile:profile,careerLength,debutYear,retirementAge,careerFactor:.85,annualShape:1,form:randInt(random,48,72),raceShape:randInt(random,20,35),fatigue:0,raceDays:0,lastRaceDay:0,lastConditionDay:1,busyUntilDay:0,mandatoryRestUntil:0,raceLoadLog:[],
...
function setAnnualRiderValues(rider,year){
  ...
  const random=mulberry32(hashString(`${rider.id}|${year}|condition`));rider.form=randInt(random,48,72);rider.raceShape=randInt(random,20,35);rider.fatigue=0;rider.raceDays=0;rider.lastRaceDay=0;rider.lastConditionDay=1;rider.busyUntilDay=0;rider.mandatoryRestUntil=0;rider.raceLoadLog=[];rider.currentSeason=freshStats();rider.targetEvents=[];
}
...
function eventDuration(event){return Math.max(1,event.stageProfiles?.length||1);}
function endDay(state,event){return startDay(state,event)+eventDuration(event)-1;}
function postRaceRecoveryDays(event){const days=eventDuration(event);if(event.kind==='grand-tour')return 18;if(days>=7)return 9;if(days>=4)return 6;if(event.kind==='monument'||event.kind==='championship')return 3;return 2;}
function recentRaceDays(rider,day,window=28){return (rider.raceLoadLog||[]).filter(row=>row.year===rider.skillYear&&row.end>=day-window&&row.start<=day).reduce((sum,row)=>sum+(row.days||1),0);}
function recentRaceCount(rider,day,window=30){return (rider.raceLoadLog||[]).filter(row=>row.year===rider.skillYear&&row.end>=day-window&&row.start<=day).length;}
...
export function createUniverse({name='Main Chronicle',seed=20260728}={}){
  ...
  const state={version:16,resultConsistencyV11:true,name,seed,year:2026,currentDay:1,eventIndex:0,seasonStatus:'active',reviewMode:false,pendingArchive:null,teams,riders,directors,directorAgencies:agencies,events,eventResults:[],currentResult:null,news:[],archives:[],transfers:[],directorMoves:[],sponsorLog:[],tierChanges:[],retirements:[],prospectSpawns:[],watchlist:{riders:[],teams:[],directors:[],races:[],countries:[]},weeklyRankings:[],rankingMilestones:[],graduationClasses:[],settings:{autosave:true,stageDetail:'winners'},createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};
...
const LEGACY_RARITY_RANGES = {
  generational:[94,98], legend:[89,94], epic:[84,90], rare:[78,85], uncommon:[72,80], common:[65,76]
};
function normalizedBaseSkill(rider,previousVersion){
  const range=RARITIES[rider.rarity]||RARITIES.common,raw=Number(rider.baseSkill??rider.potential??range.min);
  if(previousVersion>=9)return clamp(Math.round(raw),range.min,range.max);
  const legacy=LEGACY_RARITY_RANGES[rider.rarity]||[50,100],ratio=clamp((raw-legacy[0])/Math.max(1,legacy[1]-legacy[0]),0,1);
...
export function upgradeUniverse(state){
  ...
  for(const rider of state.riders||[]){
    ...
    rider.debutAge=rider.debutAge??Math.max(18,rider.age-(state.year-rider.debutYear));const debutAge=rider.debutAge;
    if(!rider.retirementAge||rider.retirementAge<28||!rider.careerLength||rider.careerLength<10){const rr=mulberry32(hashString(`${rider.id}|career-v6`));rider.retirementAge=retirementAgeFor(rr,rider.rarity);rider.careerLength=Math.max(10,rider.retirementAge-debutAge);}
    ...
    rider.raceShape=rider.raceShape??rider.form??30;rider.lastRaceDay=rider.lastRaceDay??0;rider.lastConditionDay=rider.lastConditionDay??Math.max(1,rider.lastRaceDay||1);rider.busyUntilDay=rider.busyUntilDay??rider.lastRaceDay??0;rider.mandatoryRestUntil=rider.mandatoryRestUntil??0;rider.raceLoadLog=Array.isArray(rider.raceLoadLog)?rider.raceLoadLog.filter(row=>row&&row.year===state.year):[];rider.happiness=rider.happiness??rider.morale??65;rider.contractYears=rider.contractYears||2;rider.currentSeason=normalizeStats(rider.currentSeason);rider.career=normalizeCareer(rider.career);
  ...
  for(const archive of state.archives||[]){
    archive.riderRanking=archive.riderRanking||[];archive.teamRanking=archive.teamRanking||[];archive.directorRanking=archive.directorRanking||[];archive.raceWinners=archive.raceWinners||[];archive.u23Ranking=archive.u23Ranking||[];
...
function replaceCurrentSeasonArchive(state,entity,book,extra={}){
  const season=entity.career?.seasons?.find(item=>item.year===state.year);if(!season)return;
  Object.assign(season,seasonSnapshot(book),extra);
}
export function reconcileCurrentSeasonResults(state,{force=false}={}){
  if(!state?.eventResults?.length)return state;
  const yearResults=state.eventResults.filter(result=>result.year===state.year);
  const pointRows=(state.uciPointEvents||[]).filter(award=>award.year===state.year);
  const stamp=`${state.year}:${yearResults.length}:${pointRows.length}`;
  if(!force&&state.currentSeasonConsistencyStamp===stamp)return state;
  const riderBooks=new Map((state.riders||[]).map(rider=>[rider.id,freshStats()]));
  const teamBooks=new Map((state.teams||[]).map(team=>[team.id,freshStats()]));
  const directorBooks=new Map((state.directors||[]).map(director=>[director.id,freshStats()]));
  const directorBookForTeam=teamId=>{const team=teamById(state,teamId);return team?.directorId?directorBooks.get(team.directorId):null;};
  for(const result of yearResults){
    const event=eventById(state,result.eventId);if(!event)continue;const days=Math.max(1,event.stageProfiles?.length||result.stages?.length||1);
    const participants=[...new Set(result.participantIds||[])];const participatingTeams=[...new Set((result.teams||[]).filter(Boolean))];
    for(const riderId of participants){const book=riderBooks.get(riderId);if(book){increment(book,'starts',1,event.tier);increment(book,'raceDays',days,event.tier);}}
    for(const teamId of participatingTeams){const book=teamBooks.get(teamId);if(book){increment(book,'starts',1,event.tier);increment(book,'raceDays',days,event.tier);}const directorBook=directorBookForTeam(teamId);if(directorBook){increment(directorBook,'starts',1,event.tier);increment(directorBook,'raceDays',days,event.tier);}}
    const winnerBook=riderBooks.get(result.winnerId);if(winnerBook)recordWin(winnerBook,event,state.year);
    const winnerTeamBook=teamBooks.get(result.winnerTeamId);if(winnerTeamBook)recordWin(winnerTeamBook,event,state.year);
    const winnerDirectorBook=directorBookForTeam(result.winnerTeamId);if(winnerDirectorBook)recordWin(winnerDirectorBook,event,state.year);
    for(const stage of result.stages||[]){const riderBook=riderBooks.get(stage.winnerId);if(riderBook)recordStage(riderBook,event,stage.profile,state.year);const teamId=stage.teamId||riderById(state,stage.winnerId)?.teamId;const teamBook=teamBooks.get(teamId);if(teamBook)recordStage(teamBook,event,stage.profile,state.year);const directorBook=directorBookForTeam(teamId);if(directorBook)recordStage(directorBook,event,stage.profile,state.year);}
    for(const[type,riderId]of Object.entries(result.jerseys||{})){if(!riderId)continue;const riderBook=riderBooks.get(riderId);if(riderBook)recordJersey(riderBook,event,type,state.year);const teamId=riderById(state,riderId)?.teamId;const teamBook=teamBooks.get(teamId);if(teamBook)recordJersey(teamBook,event,type,state.year);const directorBook=directorBookForTeam(teamId);if(directorBook)recordJersey(directorBook,event,type,state.year);}
    if(result.teamClassification){const teamBook=teamBooks.get(result.teamClassification);if(teamBook)recordTeamClassification(teamBook,event,state.year);const directorBook=directorBookForTeam(result.teamClassification);if(directorBook)recordTeamClassification(directorBook,event,state.year);for(const riderId of participants){if(riderById(state,riderId)?.teamId===result.teamClassification){const book=riderBooks.get(riderId);if(book)recordTeamClassification(book,event,state.year);}}}
    for(const row of (result.classification||[]).slice(0,10)){const book=riderBooks.get(row.riderId);if(!book)continue;if(row.rank<=3)increment(book,'podiums',1,event.tier);increment(book,'top10',1,event.tier);const rec={eventId:event.id,event:event.name,rank:row.rank,year:state.year,tier:event.tier,kind:event.kind};book.bestResults.push(rec);if(event.kind==='grand-tour')book.grandTourResults.push({eventId:event.id,event:event.name,rank:row.rank});}
  }
  for(const award of pointRows){const riderBook=riderBooks.get(award.riderId),event=eventById(state,award.eventId)||{id:award.eventId,name:award.eventName,tier:award.tier};if(riderBook){increment(riderBook,'uciPoints',award.points,award.tier);recordUciPointDetail(riderBook,event,award.points,award.category);}const teamBook=teamBooks.get(award.teamId);if(teamBook)increment(teamBook,'uciPoints',award.points,award.tier);const directorBook=directorBookForTeam(award.teamId);if(directorBook)increment(directorBook,'uciPoints',award.points,award.tier);}
  for(const rider of state.riders||[]){const book=riderBooks.get(rider.id);if(!book)continue;rider.currentSeason=book;replaceCurrentSeasonArchive(state,rider,book,{teamId:rider.teamId,teamName:teamById(state,rider.teamId)?.name||'Independent',tier:rider.tier,grandTourResults:structuredClone(book.grandTourResults)});}
  for(const team of state.teams||[]){const book=teamBooks.get(team.id);if(!book)continue;team.currentSeason=book;team.uciPoints=book.uciPoints;replaceCurrentSeasonArchive(state,team,book,{name:team.name,tier:team.tier,directorId:team.directorId});}
  for(const director of state.directors||[]){const book=directorBooks.get(director.id);if(!book)continue;director.currentSeason=book;replaceCurrentSeasonArchive(state,director,book,{teamId:director.teamId,teamName:director.teamId?teamById(state,director.teamId)?.name:'Agency',tier:director.teamId?teamById(state,director.teamId)?.tier:'agency'});}
  state.currentSeasonConsistencyStamp=stamp;return state;
}
...
function prepareSeason(state){
  state.eventIndex=0;state.currentDay=1;state.currentResult=null;state.eventResults=[];state.currentSeasonConsistencyStamp=null;state.seasonStatus='active';state.reviewMode=false;state.pendingArchive=null;
...
function planCalendars(state){
  ...
  for(const rider of state.riders){
    ...
    const candidates=activeEvents.filter(event=>eventEligibleForRider(event,rider)),scored=candidates.map(event=>({event,score:eventTargetScore(event,rider)+deterministicNoise(state,rider.id,event.id)*8})).sort((a,b)=>b.score-a.score),calendarRandom=mulberry32(hashString(`${rider.id}|${state.year}|calendar-v15`)),elite=ELITE_RARITIES.includes(rider.rarity),targetDays=elite?(rider.program==='monuments'?randInt(calendarRandom,48,62):rider.program==='stage-hunter'?randInt(calendarRandom,56,68):randInt(calendarRandom,54,68)):rider.tier==='u23'?randInt(calendarRandom,32,46):rider.tier==='continental'?randInt(calendarRandom,34,50):rider.role==='Domestique'?randInt(calendarRandom,70,86):rider.program==='stage-hunter'?randInt(calendarRandom,62,76):rider.program==='monuments'?randInt(calendarRandom,50,64):randInt(calendarRandom,55,70),targetCap=elite?(rider.program==='monuments'?14:rider.program==='stage-hunter'?12:11):rider.role==='Domestique'?18:rider.program==='monuments'?16:rider.program==='stage-hunter'?14:rider.program==='one-week'?12:11,minTargets=rider.tier==='u23'?7:rider.tier==='continental'?7:elite?8:rider.role==='Domestique'?12:8,maxGrandTours=rider.program==='grand-tour'?(rider.rarity==='generational'?2:rider.rarity==='legend'&&calendarRandom()<.35?2:1):1;
    let days=0,grandTours=0;const selected=[];
    for(const{event}of scored){
      if(days>=targetDays||selected.length>=targetCap)break;
      if(event.kind==='grand-tour'&&grandTours>=maxGrandTours)continue;
      if(!calendarLoadAllows(state,event,selected,rider))continue;
      selected.push(event);days+=eventDuration(event);if(event.kind==='grand-tour')grandTours++;
    }
    if(rider.rarity==='generational'){
      for(const kind of [rider.program==='grand-tour'?'monument':'grand-tour','stage']){
        const option=candidates.filter(event=>event.kind===kind&&!selected.includes(event)&&event.prestige>=86&&calendarLoadAllows(state,event,selected,rider)).sort((a,b)=>eventTargetScore(b,rider)-eventTargetScore(a,rider))[0];
        if(option&&selected.length<targetCap){selected.push(option);days+=eventDuration(option);}
      }
    }
    if(selected.length<minTargets){for(const{event}of scored){if(selected.length>=minTargets||selected.length>=targetCap)break;if(selected.includes(event)||event.kind==='grand-tour'||!calendarLoadAllows(state,event,selected,rider))continue;selected.push(event);}}
    rider.targetEvents=selected.sort((a,b)=>startDay(state,a)-startDay(state,b)).map(event=>event.id);
  }
}
function calendarLoadAllows(state,event,selected,rider){
  const start=startDay(state,event),finish=endDay(state,event),duration=eventDuration(event),elite=ELITE_RARITIES.includes(rider.rarity)||rider.role!=='Domestique';
  for(const target of selected){const ts=startDay(state,target),te=endDay(state,target),buffer=Math.max(postRaceRecoveryDays(target),postRaceRecoveryDays(event));if(start<=te+buffer&&finish>=ts-buffer)return false;}
  const nearby=selected.filter(target=>endDay(state,target)>=start-29&&startDay(state,target)<=finish+29),priorWindowDays=nearby.reduce((sum,target)=>sum+eventDuration(target),0),windowDays=priorWindowDays+duration,windowEvents=nearby.length+1;
  if(event.kind==='grand-tour'){if(priorWindowDays>0)return false;}else if(windowDays>(elite?18:22))return false;
  if(windowEvents>(rider.program==='monuments'?6:5))return false;
  const sameMonth=selected.filter(target=>target.month===event.month),priorMonthDays=sameMonth.reduce((sum,target)=>sum+eventDuration(target),0),monthDays=priorMonthDays+duration;
  if(event.kind==='grand-tour'){if(priorMonthDays>0)return false;}else if(monthDays>(elite?19:23))return false;
  return true;
}
...
function riderAvailableForEvent(state,rider,event,{allowEmergency=false}={}){const start=startDay(state,event),targeted=(rider.targetEvents||[]).includes(event.id),elite=ELITE_RARITIES.includes(rider.rarity)||rider.role!=='Domestique',load=recentRaceDays(rider,start,28),count=recentRaceCount(rider,start,30),duration=eventDuration(event);if((rider.busyUntilDay||0)>=start||(rider.mandatoryRestUntil||0)>=start)return false;if(event.kind==='grand-tour'){if(load>0)return false;}else if(load+duration>(elite?18:22))return false;if(count+1>(rider.program==='monuments'?6:5)||rider.fatigue>=76)return false;if(!targeted&&!allowEmergency&&elite)return false;return true;}
...
function selectionScore(state,rider,event){const targeted=rider.targetEvents.includes(event.id),eventFit=Math.max(...event.stageProfiles.map(type=>stageSkillRating(rider,type))),shape=rider.raceShape??35,load=recentRaceDays(rider,startDay(state,event),28),targetBonus=targeted?32:(ELITE_RARITIES.includes(rider.rarity)||rider.role!=='Domestique'?-36:-8),fatiguePenalty=(rider.fatigue||0)*1.05+load*.55,readiness=(shape-45)*.34;return eventFit+targetBonus+readiness+rider.morale*.03-fatiguePenalty+deterministicNoise(state,rider.id,event.id)*5;}
function nationalSelections(state,event,u23=false){const pool=state.riders.filter(r=>!r.retired&&(u23?r.age<=22:!['u23','continental'].includes(r.tier))&&riderAvailableForEvent(state,r,event,{allowEmergency:false})),nations=[...new Set(pool.map(r=>r.nationality))];return nations.map(nation=>({id:`${u23?'u23-':''}nation-${nation}`,name:`${nation} ${u23?'U23 ':''}Selection`,nationality:nation,tier:u23?'u23':'national',roster:pool.filter(r=>r.nationality===nation).sort((a,b)=>selectionScore(state,b,event)-selectionScore(state,a,event)).slice(0,u23?6:8).map(r=>r.id)})).filter(t=>t.roster.length>=3).sort((a,b)=>b.roster.length-a.roster.length).slice(0,EVENT_TEAM_LIMIT[event.kind]||22);}
...
function rosterForTeam(state,team,event){const size=event.tier==='u23'?6:event.kind==='grand-tour'?8:7,base=team.roster.map(id=>riderById(state,id)).filter(r=>r&&!r.retired&&r.injuryWeeks===0&&eventEligibleForRider(event,r)),targeted=base.filter(r=>riderAvailableForEvent(state,r,event)&&r.targetEvents.includes(event.id)).map(r=>({r,score:selectionScore(state,r,event)})).sort((a,b)=>b.score-a.score),safeFillers=base.filter(r=>!targeted.some(x=>x.r.id===r.id)&&riderAvailableForEvent(state,r,event,{allowEmergency:r.role==='Domestique'&&!ELITE_RARITIES.includes(r.rarity)})).map(r=>({r,score:selectionScore(state,r,event)})).sort((a,b)=>b.score-a.score);let chosen=[...targeted,...safeFillers].slice(0,size);if(chosen.length<size){const emergency=base.filter(r=>!chosen.some(x=>x.r.id===r.id)&&!ELITE_RARITIES.includes(r.rarity)&&r.role==='Domestique'&&riderAvailableForEvent(state,r,event,{allowEmergency:true})&&r.fatigue<68).map(r=>({r,score:selectionScore(state,r,event)-25})).sort((a,b)=>b.score-a.score);chosen=chosen.concat(emergency.slice(0,size-chosen.length));}return chosen.map(x=>x.r);}
function stagePerformance(state,rider,profile,event,team,randomness){
  const skill=stageSkillRating(rider,profile),ability=currentAbility(rider),programKind=event.kind==='grand-tour'?'grand-tour':['stage','u23-stage'].includes(event.kind)?'stage':event.kind==='monument'?'monument':['championship','u23-championship'].includes(event.kind)?'championship':'one-day',rawProgram=PROGRAM_EVENT_WEIGHTS[rider.program]?.[programKind]||1,versatility=ability>=95?.36:ability>=91?.52:ability>=87?.72:.9,program=1+(rawProgram-1)*versatility,roleBonus=rider.role==='Leader'&&['mountain','time-trial'].includes(profile)?1.018:rider.role==='Sprinter'&&profile==='flat'?1.035:rider.role==='Classics leader'&&['cobbles','hilly','puncheur'].includes(profile)?1.028:1,targetBonus=rider.targetEvents.includes(event.id)?1.035:.987,freshness=clamp(1-rider.fatigue/360,.77,1.02),director=team?.directorId?directorById(state,team.directorId):null,directorBonus=director?1+(director.tactics-70)/720+(director.development-70)/2200:1,facilityBonus=team?1+((team.facilities||5)-6.5)/230:1;
...
export function uciRankings(state,scope='rolling',asOfDay=null){
  if(!state||Number(state.version||0)<16)upgradeUniverse(state);const day=clamp(Math.round(asOfDay??state.currentDay??1),1,365),date=dateFromDay(state.year,day),now=Math.floor(Date.UTC(state.year,date.month-1,date.day)/86400000),min=scope==='rolling'?now-364:Math.floor(Date.UTC(state.year,0,1)/86400000),max=scope==='rolling'?now:Math.floor(Date.UTC(state.year,11,31)/86400000),awards=(state.uciPointEvents||[]).filter(a=>a.day>=min&&a.day<=max),riders=new Map();for(const a of awards)riders.set(a.riderId,(riders.get(a.riderId)||0)+a.points);const riderRows=[...riders.entries()].map(([id,points])=>({id,points,rider:riderById(state,id)})).filter(x=>x.rider).sort((a,b)=>b.points-a.points).map((x,i)=>({rank:i+1,id:x.id,name:x.rider.name,nationality:x.rider.nationality,teamId:x.rider.teamId,tier:x.rider.tier,age:x.rider.age,points:x.points}));const teamMap=new Map();for(const row of riderRows){const id=row.teamId;if(!id)continue;if(!teamMap.has(id))teamMap.set(id,[]);teamMap.get(id).push(row.points);}const teamRows=[...teamMap.entries()].map(([id,values])=>({id,team:teamById(state,id),points:values.sort((a,b)=>b-a).slice(0,20).reduce((a,b)=>a+b,0)})).filter(x=>x.team).sort((a,b)=>b.points-a.points).map((x,i)=>({rank:i+1,id:x.id,name:x.team.name,nationality:x.team.nationality,tier:x.team.tier,points:x.points}));return{riders:riderRows,teams:teamRows,scope,year:state.year,day};
}
function compactRankingRows(rows,limit){return rows.slice(0,limit).map(row=>[row.id,row.points,row.rank,row.teamId||null,row.tier||null]);}
function previousSnapshot(state,year,week){return [...(state.weeklyRankings||[])].filter(row=>row.year<year||(row.year===year&&row.week<week)).sort((a,b)=>b.year-a.year||b.week-a.week)[0]||null;}
function snapshotMap(rows=[]){return new Map(rows.map(row=>[row[0],{id:row[0],points:row[1],rank:row[2],teamId:row[3],tier:row[4],nationality:row[5]}]));}
function addRankingMilestone(state,riderId,type,year,week,rank){if(state.rankingMilestones.some(row=>row.riderId===riderId&&row.type===type))return;state.rankingMilestones.push({riderId,type,year,week,rank});state.rankingMilestones=state.rankingMilestones.slice(-3000);}
function publishRankingWatch(state,snapshot,previous){if(!previous)return;const currentRows=snapshotMap(snapshot.riders),previousRows=snapshotMap(previous.riders),currentOne=snapshot.riders[0]?.[0],previousOne=previous.riders[0]?.[0],storyId=`ranking-${snapshot.year}-${snapshot.week}`;if(currentOne&&previousOne&&currentOne!==previousOne&&!state.news.some(row=>row.id===storyId+'-one')){const rider=riderById(state,currentOne);state.news.unshift({id:storyId+'-one',year:snapshot.year,phase:'season',category:'Ranking Watch',importance:'major',headline:`${rider?.name||'A new leader'} takes world number one`,body:`The rolling table changed hands in week ${snapshot.week}. ${rider?.name||'The new leader'} moved to the summit with ${currentRows.get(currentOne)?.points||0} points.`,riderIds:[currentOne,previousOne]});}
  const newcomer=[...currentRows.values()].filter(row=>row.rank<=10&&(previousRows.get(row.id)?.rank||999)>10).sort((a,b)=>a.rank-b.rank)[0];if(newcomer&&!state.news.some(row=>row.id===storyId+'-top10')){const rider=riderById(state,newcomer.id);state.news.unshift({id:storyId+'-top10',year:snapshot.year,phase:'season',category:'Ranking Watch',importance:'normal',headline:`${rider?.name||'A rider'} enters the world top ten`,body:`A strong run of results lifted ${rider?.name||'the rider'} to number ${newcomer.rank} in week ${snapshot.week}.`,riderIds:[newcomer.id]});}
  state.news=state.news.slice(0,220);
}
export function captureWeeklyRankingSnapshot(state,day=state.currentDay||1,{force=false,silent=false}={}){
  const clamped=clamp(Math.round(day||1),1,365),week=Math.ceil(clamped/7),existingIndex=(state.weeklyRankings||[]).findIndex(row=>row.year===state.year&&row.week===week);if(existingIndex>=0&&!force)return state.weeklyRankings[existingIndex];const ranking=uciRankings(state,'rolling',clamped),snapshot={year:state.year,week,day:clamped,riders:compactRankingRows(ranking.riders,100),teams:compactRankingRows(ranking.teams,30)},previous=previousSnapshot(state,state.year,week);if(existingIndex>=0)state.weeklyRankings[existingIndex]=snapshot;else state.weeklyRankings.push(snapshot);state.weeklyRankings.sort((a,b)=>a.year-b.year||a.week-b.week);
  for(const row of snapshot.riders){if(row[2]<=100)addRankingMilestone(state,row[0],'top100',state.year,week,row[2]);if(row[2]<=10)addRankingMilestone(state,row[0],'top10',state.year,week,row[2]);if(row[2]===1)addRankingMilestone(state,row[0],'number-one',state.year,week,1);}if(!silent)publishRankingWatch(state,snapshot,previous);return snapshot;
}
export function rankingSnapshotRows(state,snapshot,entity='riders'){
  const rows=snapshot?.[entity]||[],previous=previousSnapshot(state,snapshot?.year||state.year,snapshot?.week||1),previousRows=snapshotMap(previous?.[entity]||[]);return rows.map(row=>{const source=entity==='riders'?riderById(state,row[0]):teamById(state,row[0]),old=previousRows.get(row[0]),movement=old?old.rank-row[2]:null,pointsChange=old?row[1]-old.points:null;return{rank:row[2],id:row[0],name:source?.name||row[0],nationality:source?.nationality||row[5],teamId:row[3],tier:row[4]||source?.tier,points:row[1],previousRank:old?.rank||null,movement,pointsChange};});
}
export function riderRankingHistory(state,riderId){return (state.weeklyRankings||[]).map(snapshot=>{const row=snapshot.riders.find(item=>item[0]===riderId);return row?{year:snapshot.year,week:snapshot.week,day:snapshot.day,rank:row[2],points:row[1]}:null;}).filter(Boolean);}
export function riderRivalries(state,riderId,limit=10){
  const map=new Map();for(const event of state.events||[])for(const edition of event.editions||[]){const rows=edition.top10||[],self=rows.find(row=>(row.riderId||row.id)===riderId);if(!self)continue;for(const opponent of rows){const opponentId=opponent.riderId||opponent.id;if(!opponentId||opponentId===riderId)continue;const entry=map.get(opponentId)||{id:opponentId,meetings:0,ahead:0,behind:0,majorMeetings:0,podiumMeetings:0,closeBattles:0,score:0,events:[]},selfRank=self.rank||rows.indexOf(self)+1,opponentRank=opponent.rank||rows.indexOf(opponent)+1,major=['grand-tour','monument','championship'].includes(event.kind),close=Math.abs((self.gap||self.time||0)-(opponent.gap||opponent.time||0))<=60||Math.abs(selfRank-opponentRank)<=1;entry.meetings++;if(selfRank<opponentRank)entry.ahead++;else entry.behind++;if(major)entry.majorMeetings++;if(selfRank<=3&&opponentRank<=3)entry.podiumMeetings++;if(close)entry.closeBattles++;entry.score+=2+(major?8:0)+(close?4:0)+(selfRank<=3&&opponentRank<=3?5:0);entry.events.push({year:edition.year,eventId:event.id,event:event.name,selfRank,opponentRank,major});map.set(opponentId,entry);}}
  return [...map.values()].map(entry=>{const rider=riderById(state,entry.id);return{...entry,name:rider?.name||entry.id,nationality:rider?.nationality,tier:rider?.tier,latest:entry.events.sort((a,b)=>b.year-a.year)[0]};}).filter(row=>row.meetings>=2||row.majorMeetings>=1).sort((a,b)=>b.score-a.score||b.meetings-a.meetings).slice(0,limit);
}
export function riderRecordBook(state,riderId){
  const rider=riderById(state,riderId);if(!rider)return{};const history=riderRankingHistory(state,riderId),wins=rider.career?.raceWinDetails||[],stages=rider.career?.stageWinDetails||[],seasons=rider.career?.seasons||[],raceCounts=new Map();for(const win of wins)raceCounts.set(win.eventId,{event:win.event,count:(raceCounts.get(win.eventId)?.count||0)+1});const signature=[...raceCounts.values()].sort((a,b)=>b.count-a.count)[0]||null;const stageRaceCounts=new Map();for(const win of stages)stageRaceCounts.set(win.eventId,{event:win.event,count:(stageRaceCounts.get(win.eventId)?.count||0)+1});const mostStages=[...stageRaceCounts.values()].sort((a,b)=>b.count-a.count)[0]||null;const bestSeason=[...seasons].sort((a,b)=>(b.points||0)-(a.points||0))[0]||null;const winningYears=seasons.filter(row=>(row.raceWins||0)>0).map(row=>row.year).sort((a,b)=>a-b);let longest=0,current=0,last=null;for(const year of winningYears){current=last!==null&&year===last+1?current+1:1;longest=Math.max(longest,current);last=year;}return{peakRank:history.length?Math.min(...history.map(row=>row.rank)):null,weeksAtOne:history.filter(row=>row.rank===1).length,bestSeason,signatureRace:signature,mostStagesInRace:mostStages,longestWinningStreak:longest,highestPoints:history.length?Math.max(...history.map(row=>row.points)):0,firstTop10:state.rankingMilestones.find(row=>row.riderId===riderId&&row.type==='top10')||null,firstNumberOne:state.rankingMilestones.find(row=>row.riderId===riderId&&row.type==='number-one')||null};
}
...
function recoverBeforeEvent(state,event){
  ...
  for(const rider of state.riders){
    if(rider.retired)continue;
    const from=Math.max(1,rider.lastConditionDay??1),delta=Math.max(0,eventDay-from);if(!delta)continue;
    const recoveryRate=(rider.fatigue||0)>75?.72:(rider.fatigue||0)>45?.92:1.18;rider.fatigue=clamp((rider.fatigue||0)-delta*recoveryRate,0,100);
    const daysSinceRace=Math.max(0,eventDay-(rider.lastRaceDay||1));let shape=rider.raceShape??30;
    if(daysSinceRace<=7)shape+=delta*.18;else if(daysSinceRace<=21)shape-=delta*.05;else if(daysSinceRace<=35)shape-=delta*.42;else if(daysSinceRace<=56)shape-=delta*.72;else shape-=delta*.95;
    if(rider.targetEvents.includes(event.id)&&daysSinceRace>=7&&daysSinceRace<=24)shape+=Math.min(3,delta*.22);
    rider.raceShape=clamp(shape,12,95);rider.form=clamp(rider.form+(rider.targetEvents.includes(event.id)?Math.min(1.5,delta*.05):0),35,90);rider.injuryWeeks=Math.max(0,rider.injuryWeeks-Math.floor(delta/7));rider.lastConditionDay=eventDay;
  }
}
function eventFatigueLoad(rider,event){const days=eventDuration(event),recovery=rider.skills?.recovery??riderSkillAverage(rider),recoveryFactor=clamp(1.18-recovery/180,.62,.96),base=days===1?(9+event.prestige/22+(event.stageProfiles[0]==='mountain'||event.stageProfiles[0]==='cobbles'?2:0)):days*(event.kind==='grand-tour'?3.45:3.0)+Math.max(0,days-5)*.55;return base*recoveryFactor;}
...
export function simulateNextEvent(state){
  ...
  for(let index=0;index<event.stageProfiles.length;index+=1){
    ...
    let ranked=participants.map(entry=>{
      ...
      const softCap=eliteAllRounder?(entry.rider.rarity==='generational'?6:5):entry.rider.rarity==='generational'?5:entry.rider.rarity==='legend'?4:entry.rider.rarity==='epic'?3:2,repeatPenalty=repeat<softCap?repeat*2.2:(repeat-softCap+1)*(eliteAllRounder?10:11)+softCap*2.2;
      const inRaceFatigue=localFatigue.get(entry.rider.id)||0;
      return{...entry,score:stagePerformance(state,entry.rider,profile,event,entry.team,(random()-.5)*stageNoise)-repeatPenalty-inRaceFatigue*.38};
    ...
    ranked.forEach((entry,rank)=>{
      ...
      if(profile==='mountain'){const mp=([40,28,20,14,10,7,5,3,2,1][rank]||0)*climbFactor;mountains.set(entry.rider.id,(mountains.get(entry.rider.id)||0)+mp);}
      else if(profile==='hilly'||profile==='puncheur'){const mp=([7,4,2,1][rank]||0)*climbFactor;mountains.set(entry.rider.id,(mountains.get(entry.rider.id)||0)+mp);}
    });
  ...
  const rawFinal=[...gc.entries()].map(([riderId,time])=>{const rider=riderById(state,riderId);if(!rider)return null;const ability=currentAbility(rider),gcQuality=grandTourGcRating(rider),base=rider.baseSkill??rider.potential,terrainBase={sprinter:22000,cobbles:18000,rouleur:12000,puncheur:4800,'time-trialist':1400,climber:0,'all-rounder':0}[rider.terrain]??7000,exceptionScale=rider.rarity==='generational'?clamp((98-base)/10,.12,1):clamp((98-base)/10,.35,1),terrainTax=terrainBase*exceptionScale,rarityGcTax={rare:1800,uncommon:6500,common:10000}[rider.rarity]||0,qualityTax=event.kind==='grand-tour'?Math.max(0,88-gcQuality)*620+Math.max(0,83-ability)*320+Math.pow(Math.max(0,91-base),2)*55+(rider.program==='grand-tour'?0:Math.max(0,89-gcQuality)*95)+terrainTax+rarityGcTax:0;return{rider,time:time+qualityTax};}).filter(Boolean).sort((a,b)=>a.time-b.time);
  ...
  const teamClassification=Number.isFinite(teamTotals[0]?.time)?teamTotals[0].teamId:null,winnerTeam=teamById(state,winner.teamId),winnerDirector=winnerTeam?.directorId?directorById(state,winnerTeam.directorId):null,routeProfile=event.stageProfiles.length>1?event.stageProfiles.filter(profile=>['mountain','hilly','puncheur','time-trial'].includes(profile)).at(-1)||event.stageProfiles.at(-1):event.stageProfiles[0],winnerContext={shape:Math.round(winner.raceShape||0),fatigue:Math.round(winner.fatigue||0),annualRating:Math.round(currentAbility(winner)),terrainFit:Math.round(stageSkillRating(winner,routeProfile||'hilly')),teamSupport:Math.round(teamSupport.get(winner.teamId)||0),directorTactics:winnerDirector?.tactics||null,targeted:winner.targetEvents.includes(event.id),profile:routeProfile};
  const result={id:uniqueId('result',random),year:state.year,eventId:event.id,eventName:event.name,tier:event.tier,kind:event.kind,prestige:event.prestige,stages,winnerId:winner.id,winnerName:winner.name,winnerTeamId:winner.teamId,classification:final.slice(0,20).map((entry,rank)=>({rank:rank+1,riderId:entry.rider.id,name:entry.rider.name,teamId:entry.rider.teamId,time:entry.time,gap:entry.gap})),jerseys:{points:pointsWinner,mountains:mountainWinner,young:youngWinner},teamClassification,context:winnerContext,teams:teams.map(t=>t.id),participantIds:participants.map(entry=>entry.rider.id),month:event.month,day:event.day};
  ...
  const eventEnd=endDay(state,event);
  for(const{rider}of participants){rider.lastRaceDay=eventEnd;rider.lastConditionDay=eventEnd;rider.busyUntilDay=eventEnd;rider.raceLoadLog=rider.raceLoadLog||[];rider.raceLoadLog.push({year:state.year,eventId:event.id,start:startDay(state,event),end:eventEnd,days:eventDuration(event)});rider.raceLoadLog=rider.raceLoadLog.slice(-40);}
  state.lastRaceDay=eventEnd;state.currentDay=Math.max(state.currentDay,eventEnd);return{state,result,results:[result],seasonComplete:false};
}
...
function commitEvent(state,event,result,participants,final){
  ...
  for(const{rider}of participants){increment(rider.currentSeason,'starts',1,event.tier);increment(rider.career,'starts',1,event.tier);}
  final.slice(0,10).forEach((entry,index)=>{const rider=entry.rider;if(index<3){increment(rider.currentSeason,'podiums',1,event.tier);increment(rider.career,'podiums',1,event.tier);}increment(rider.currentSeason,'top10',1,event.tier);increment(rider.career,'top10',1,event.tier);const rec={eventId:event.id,event:event.name,rank:index+1,year:state.year,tier:event.tier,kind:event.kind};rider.currentSeason.bestResults.push(rec);rider.currentSeason.bestResults.sort((a,b)=>resultValue(b,state)-resultValue(a,state));rider.currentSeason.bestResults=rider.currentSeason.bestResults.slice(0,10);if(index<5)rider.career.results.push({...rec,prestige:event.prestige});rider.career.results=rider.career.results.slice(-180);if(event.kind==='grand-tour')rider.currentSeason.grandTourResults.push({eventId:event.id,event:event.name,rank:index+1});});
  for(const{rider}of participants){const days=eventDuration(event);rider.raceDays+=days;increment(rider.currentSeason,'raceDays',days,event.tier);increment(rider.career,'raceDays',days,event.tier);rider.fatigue=clamp((rider.fatigue||0)+eventFatigueLoad(rider,event),0,100);const shapeGain=days===1?4:Math.min(16,days*1.35),fatigueDrag=Math.max(0,rider.fatigue-62)*.12;rider.raceShape=clamp((rider.raceShape??30)+shapeGain+(rider.targetEvents.includes(event.id)?2:0)-fatigueDrag,12,95);rider.form=clamp(rider.form+(rider.targetEvents.includes(event.id)?.7:0)-Math.max(0,rider.fatigue-72)*.08,30,92);const finish=endDay(state,event);rider.mandatoryRestUntil=Math.max(rider.mandatoryRestUntil||0,postEventRest(rider,event,finish));}
  ...
  state.currentSeasonConsistencyStamp=null;state.news.unshift(buildEventStory(state,event,result,final));state.news=state.news.slice(0,180);
}
...
export function simulateWeeks(state,weeks=1){
  upgradeUniverse(state);if(state.seasonStatus==='complete')return{state,results:[],seasonComplete:true,archive:state.pendingArchive};const results=[],start=state.currentDay||1,total=Math.max(1,weeks);for(let step=0;step<total&&state.seasonStatus!=='complete';step+=1){const target=Math.min(365,start+(step+1)*7);while(state.eventIndex<state.events.length&&startDay(state,state.events[state.eventIndex])<=target){const out=simulateNextEvent(state);if(out.result)results.push(out.result);if(state.seasonStatus==='complete')break;}state.currentDay=Math.max(state.currentDay,target);captureWeeklyRankingSnapshot(state,target);if(target>=365||state.eventIndex>=state.events.length){while(state.eventIndex<state.events.length){const out=simulateNextEvent(state);if(out.result)results.push(out.result);}const finish=completeSeason(state);return{...finish,results,seasonComplete:true};}}return{state,results,seasonComplete:false};
}
export function simulateSeason(state){upgradeUniverse(state);if(state.seasonStatus==='complete')return{state,results:[],seasonComplete:true,archive:state.pendingArchive};const results=[];while(state.seasonStatus!=='complete'){const output=simulateWeeks(state,1);results.push(...(output.results||[]));if(output.seasonComplete)return{...output,results};}return{state,results,seasonComplete:true,archive:state.pendingArchive};}
function seasonSnapshot(book){return{points:book.uciPoints,raceWins:book.raceWins,stageWins:book.stageWins,grandTours:book.grandTours,monuments:book.monuments,weeklong:book.weeklong,classics:book.classics,championships:book.championships,jerseys:book.jerseys,raceDays:book.raceDays,raceWinDetails:structuredClone(book.raceWinDetails||[]),stageWinDetails:structuredClone(book.stageWinDetails||[]),jerseyWinDetails:structuredClone(book.jerseyWinDetails||[]),teamClassifications:book.teamClassifications||0,teamAwardDetails:structuredClone(book.teamAwardDetails||[]),stageWinsByProfile:structuredClone(book.stageWinsByProfile||{}),uciPointDetails:structuredClone([...(book.uciPointDetails||[])].sort((a,b)=>(b.points||0)-(a.points||0)).slice(0,8))};}
function archiveSeason(state){
  ...
  const u23Ranking=state.riders.filter(r=>!r.retired&&r.age<=22).sort((a,b)=>b.currentSeason.uciPoints-a.currentSeason.uciPoints||currentAbility(b)-currentAbility(a)).slice(0,40).map((r,index)=>({rank:index+1,id:r.id,name:r.name,nationality:r.nationality,teamId:r.teamId,points:r.currentSeason.uciPoints,baseSkill:r.baseSkill,rating:Math.round(currentAbility(r)),age:r.age}));
  const headline=riderRanking[0]?`${riderRanking[0].name} closes ${state.year} as world number one.`:'Season complete';
  return{year:state.year,riderRanking,teamRanking,directorRanking,u23Ranking,raceWinners,headline,summary:{topRider:riderRanking[0]||null,topTeam:teamRanking[0]||null,mostWins:[...riderRanking].sort((a,b)=>b.raceWins-a.raceWins)[0]||null,mostStages:[...riderRanking].sort((a,b)=>b.stageWins-a.stageWins)[0]||null}};
}
function completeSeason(state){
  reconcileCurrentSeasonResults(state);
  if(state.seasonStatus==='complete'&&state.pendingArchive)return{state,result:null,seasonComplete:true,archive:state.pendingArchive};state.currentDay=dayOfYear(state.year,12,31);captureWeeklyRankingSnapshot(state,state.currentDay,{force:true});const archive=archiveSeason(state);state.pendingArchive=archive;state.seasonStatus='complete';state.reviewMode=true;state.news.unshift({year:state.year,phase:'postseason',category:'Season review',importance:'lead',headline:archive.headline,body:`${archive.summary.topTeam?.name||'The leading team'} topped the team table. ${archive.summary.mostWins?.name||'The leading winner'} recorded the most race victories and ${archive.summary.mostStages?.name||'the leading sprinter'} collected the most stages.`});return{state,result:null,seasonComplete:true,archive};
}
...
export function openNextSeason(state){
  upgradeUniverse(state);if(state.seasonStatus!=='complete')return{state,opened:false};const archive=state.pendingArchive||archiveSeason(state);if(!state.archives.some(a=>a.year===archive.year))state.archives.push(archive);state.graduationClasses.push({year:archive.year,riders:(archive.u23Ranking||[]).filter(row=>row.age>=21).slice(0,20)});state.graduationClasses=state.graduationClasses.slice(-30);state.pendingArchive=null;state.year+=1;for(const rider of state.riders)if(!rider.retired)rider.age+=1;for(const director of state.directors)if(!director.retired)director.age+=1;
  applyRetirementsAndDevelopment(state);enforceEliteCaps(state);applyTeamLifecycle(state);applySponsors(state);applyFacilities(state);applyDirectorMarket(state);applyTransfers(state);enforceEliteCaps(state);refillEliteVacancies(state,mulberry32(hashString(`${state.seed}|${state.year}|elite-refill`)));evolveEvents(state);prepareSeason(state);compactUciPointLedger(state);captureWeeklyRankingSnapshot(state,1,{force:true,silent:true});state.news.unshift({year:state.year,phase:'preseason',category:'New season',importance:'lead',headline:`Le Grand Braquet presents the ${state.year} peloton`,body:'New sponsors, promoted prospects, director moves, tier changes and revised calendars open another season.'});return{state,opened:true,archive};
}
...
function applyTransfers(state){
  ...
  const signed=new Set();let eliteYouthMoves=0;const available=team=>[...market.filter(r=>!r.retired&&!signed.has(r.id)&&!r.teamId),...state.riders.filter(r=>!r.retired&&!signed.has(r.id)&&r.tier==='u23'&&r.age>=21),...state.riders.filter(r=>!r.retired&&!signed.has(r.id)&&r.tier==='continental')].filter(r=>!(ELITE_RARITIES.includes(r.rarity)&&r.age<23&&['u23','continental'].includes(r.tier)&&eliteYouthMoves>=3));let elitePoaches=0;const ordered=[...proTeams].sort((a,b)=>(teamNeedScore(state,b)*40+b.budget+b.attraction)-(teamNeedScore(state,a)*40+a.budget+a.attraction));
  for(const team of ordered){const targetSize=TIER_ROSTER[team.tier],aggression=teamNeedScore(state,team)*45+Math.max(0,team.budget-72);if(team.tier==='worldtour'&&elitePoaches<2&&aggression>20&&random()<.42){const candidates=poachable.filter(r=>r.teamId&&r.teamId!==team.id&&!signed.has(r.id)&&teamPayroll(state,team)+expectedSalary(r,'worldtour')<=team.salaryBudget*1.04).map(r=>({r,score:candidateScoreForTeam(state,r,team,random)+currentAbility(r)*.3})).sort((a,b)=>b.score-a.score);const choice=candidates[0]?.r;if(choice&&makeRosterRoom(state,team,choice)){const origin=choice.teamId;moveRider(state,choice,team,state.year,'poached',origin,'A richer project matched his sporting ambition');signed.add(choice.id);elitePoaches++;}}
    while(team.roster.length<targetSize){const candidates=available(team).filter(r=>team.tier==='worldtour'||!requiresWorldTourDestination(state,r)).filter(r=>team.tier!=='worldtour'||r.potential>=78||currentAbility(r)>=76).filter(r=>teamPayroll(state,team)+expectedSalary(r,team.tier)<=team.salaryBudget*1.08).map(r=>({r,score:candidateScoreForTeam(state,r,team,random)})).sort((a,b)=>b.score-a.score);let choice=candidates[0]?.r;if(!choice){const used=new Set(state.riders.map(r=>r.name));choice=makeRider(random,{teamId:null,tier:team.tier,usedNames:used,ageRange:[21,24],year:state.year});choice.marketFrom='new-pro';choice.marketReason='New professional';state.riders.push(choice);}const originTier=choice.tier,origin=choice.teamId||choice.marketFrom||'development',reason=choice.marketReason|| (choice.tier==='u23'?'Promoted from the development system':'Signed to fill a roster need');signed.add(choice.id);moveRider(state,choice,team,state.year,choice.tier==='u23'?'promotion':'transfer',origin,reason);if(ELITE_RARITIES.includes(choice.rarity)&&choice.age<23&&['u23','continental'].includes(originTier))eliteYouthMoves++;}
  }
