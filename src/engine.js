...
function normalizeStats(stats={}){
  ...
  normalized.bestResults=stats.bestResults||[];normalized.grandTourResults=stats.grandTourResults||[];
  return normalized;
}
...
function recordWin(book,event){increment(book,'raceWins',1,event.tier);increment(book,'gcWins',event.stageProfiles.length>1?1:0,event.tier);increment(book,eventMetric(event),1,event.tier);}
function recordStage(book,event,profile){increment(book,'stageWins',1,event.tier);book.stageWinsByProfile[profile]=(book.stageWinsByProfile[profile]||0)+1;}
function recordJersey(book,event){increment(book,'jerseys',1,event.tier);}
...
export function createUniverse({name='Main Chronicle',seed=20260728}={}){
  ...
  const state={version:2,name,seed,year:2026,currentDay:1,eventIndex:0,teams,riders,directors,directorAgencies:agencies,events,eventResults:[],currentResult:null,news:[],archives:[],transfers:[],directorMoves:[],sponsorLog:[],settings:{autosave:true,stageDetail:'winners'},createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};
  prepareSeason(state);openingNews(state);return state;
}

export function upgradeUniverse(state){
  if(!state)return state;
  state.version=2;state.currentDay=state.currentDay||Math.max(1,state.events?.[state.eventIndex]?dayOfYear(state.year,state.events[state.eventIndex].month,state.events[state.eventIndex].day)-1:1);state.directorMoves=state.directorMoves||[];state.directorAgencies=state.directorAgencies||[];
...
function prepareSeason(state){
  state.eventIndex=0;state.currentDay=1;state.currentResult=null;state.eventResults=[];
...
function stagePerformance(state,rider,profile,event,team,randomness){
  ...
  return ability*terrain*program*roleBonus*targetBonus*freshness*directorBonus+randomness;
}
function timeGapFromScore(top,score,profile){const factor=profile==='mountain'?19:profile==='time-trial'?13:['hilly','puncheur'].includes(profile)?8:3;return Math.max(0,Math.round((top-score)*factor));}
...
export function simulateNextEvent(state){
  upgradeUniverse(state);const event=state.events[state.eventIndex];if(!event)return finishSeason(state);recoverBeforeEvent(state,event);const random=mulberry32(hashString(`${state.seed}|${state.year}|${event.id}|simulation`)),teams=eventTeams(state,event);let participants=[];
...
function awardStageWin(state,rider,event,profile){recordStage(rider.currentSeason,event,profile);recordStage(rider.career,event,profile);const points=Math.round(event.prestige*.45);increment(rider.currentSeason,'uciPoints',points,event.tier);increment(rider.career,'uciPoints',points,event.tier);const team=teamById(state,rider.teamId);if(team){recordStage(team.currentSeason,event,profile);recordStage(team.career,event,profile);increment(team.currentSeason,'uciPoints',points,event.tier);increment(team.career,'uciPoints',points,event.tier);team.uciPoints+=points;team.cyclePoints+=points;const director=directorById(state,team.directorId);if(director){recordStage(director.currentSeason,event,profile);recordStage(director.career,event,profile);increment(director.currentSeason,'uciPoints',points,event.tier);increment(director.career,'uciPoints',points,event.tier);}}}
function commitEvent(state,event,result,participants,final){
  const winner=riderById(state,result.winnerId);recordWin(winner.currentSeason,event);recordWin(winner.career,event);if(event.id==='worlds-road'||event.id==='worlds-tt')winner.career.worlds=(winner.career.worlds||0)+1;
  const winPoints=Math.round(event.prestige*(event.kind==='grand-tour'?6:event.kind==='monument'?4:['stage','u23-stage'].includes(event.kind)?2.5:2));increment(winner.currentSeason,'uciPoints',winPoints,event.tier);increment(winner.career,'uciPoints',winPoints,event.tier);
  const winnerTeam=teamById(state,winner.teamId);if(winnerTeam){recordWin(winnerTeam.currentSeason,event);recordWin(winnerTeam.career,event);increment(winnerTeam.currentSeason,'uciPoints',winPoints,event.tier);increment(winnerTeam.career,'uciPoints',winPoints,event.tier);winnerTeam.uciPoints+=winPoints;winnerTeam.cyclePoints+=winPoints;const director=directorById(state,winnerTeam.directorId);if(director){recordWin(director.currentSeason,event);recordWin(director.career,event);increment(director.currentSeason,'uciPoints',winPoints,event.tier);increment(director.career,'uciPoints',winPoints,event.tier);}}
  for(const id of Object.values(result.jerseys).filter(Boolean)){const r=riderById(state,id);if(r){recordJersey(r.currentSeason,event);recordJersey(r.career,event);const t=teamById(state,r.teamId);if(t){recordJersey(t.currentSeason,event);recordJersey(t.career,event);const d=directorById(state,t.directorId);if(d){recordJersey(d.currentSeason,event);recordJersey(d.career,event);}}}}
  ...
  event.editions=event.editions||[];event.editions.push({year:state.year,winnerId:result.winnerId,winnerName:result.winnerName,teamId:result.winnerTeamId,tier:event.tier,top10:result.classification.slice(0,10),stageWinners:result.stages.map(s=>({riderId:s.winnerId,name:s.winnerName,teamId:s.teamId,profile:s.profile})),jerseys:result.jerseys});
...
function buildEventStory(state,event,result,final){const winner=riderById(state,result.winnerId),runner=final[1]?.rider,surprise=currentAbility(winner)<82,fatigueText=winner.raceDays>55?` after already accumulating ${winner.raceDays} race days`:' after a protected build-up',past=(event.editions||[]).filter(e=>e.winnerId===winner.id).length;let headline=`${winner.name} wins ${event.name}`;if(surprise)headline=`Shock in ${event.name}: ${winner.name} breaks through`;else if(past>1)headline=`${winner.name} masters ${event.name} again`;const body=event.stageProfiles.length>1?`${winner.name} completed the ${event.stageProfiles.length}-stage race ahead of ${runner?.name||'the field'}${fatigueText}. ${result.stages.filter(stage=>stage.winnerId===winner.id).length} stage win(s) supported the final classification.`:`${winner.name} defeated ${runner?.name||'the leading contenders'} in a ${event.stageProfiles[0].replace('-',' ')} contest.`;return{year:state.year,phase:'season',eventId:event.id,category:event.kind==='grand-tour'?'Grand Tour':event.kind==='monument'?'Monuments':event.tier==='u23'?'Young Wheels':'Race report',importance:event.prestige>=96?'lead':surprise?'major':'normal',headline,body};}

export function simulateWeeks(state,weeks=1){
  upgradeUniverse(state);const startYear=state.year,target=Math.min(366,state.currentDay+Math.max(1,weeks)*7),results=[];while(state.year===startYear&&state.eventIndex<state.events.length&&startDay(state,state.events[state.eventIndex])<=target){const out=simulateNextEvent(state);if(out.result)results.push(out.result);}
  if(state.year!==startYear)return{state,results,seasonComplete:true,archive:state.archives.at(-1)};state.currentDay=Math.max(state.currentDay,target);if(state.currentDay>=365){while(state.eventIndex<state.events.length){const out=simulateNextEvent(state);if(out.result)results.push(out.result);}const finish=finishSeason(state);return{...finish,results,seasonComplete:true};}return{state,results,seasonComplete:false};
}
export function simulateSeason(state){const year=state.year,results=[];while(state.year===year&&state.eventIndex<state.events.length){const out=simulateNextEvent(state);if(out.result)results.push(out.result);}const finish=state.year===year?finishSeason(state):{state,archive:state.archives.at(-1),seasonComplete:true};return{...finish,results};}

function seasonSnapshot(book){return{points:book.uciPoints,raceWins:book.raceWins,stageWins:book.stageWins,grandTours:book.grandTours,monuments:book.monuments,weeklong:book.weeklong,classics:book.classics,championships:book.championships,jerseys:book.jerseys,raceDays:book.raceDays};}
...
function finishSeason(state){
  const archive=archiveSeason(state);state.archives.push(archive);state.news.unshift({year:state.year,phase:'postseason',category:'Season review',importance:'lead',headline:archive.headline,body:`${archive.summary.topTeam?.name||'The leading team'} topped the team table. ${archive.summary.mostWins?.name||'The leading winner'} recorded the most race victories and ${archive.summary.mostStages?.name||'the leading sprinter'} collected the most stages.`});
  state.year+=1;for(const rider of state.riders)if(!rider.retired)rider.age+=1;for(const director of state.directors)if(!director.retired)director.age+=1;
  applyRetirementsAndDevelopment(state);applyTeamLifecycle(state);applyTransfers(state);applySponsors(state);applyDirectorMarket(state);evolveEvents(state);prepareSeason(state);state.news.unshift({year:state.year,phase:'preseason',category:'New season',importance:'lead',headline:`Le Grand Braquet presents the ${state.year} peloton`,body:'New sponsors, promoted prospects, director moves and revised calendars open another season.'});return{state,result:null,seasonComplete:true,archive};
}

export function hallScore(rider){const c=rider.career;return Math.round((c.grandTours||0)*1800+(c.monuments||0)*1100+(c.worlds||0)*1250+(c.olympics||0)*1300+(c.weeklong||0)*280+(c.classics||0)*180+(c.raceWins||0)*110+(c.stageWins||0)*55+(c.podiums||0)*20+(c.jerseys||0)*35+(c.uciPoints||0)*.15);}
...
function moveRider(state,rider,destination,year,type='transfer',fromId=null){
  ...
  rider.history.push({year,text:`Joined ${destination.name}.`});state.transfers.unshift({year,riderId:rider.id,riderName:rider.name,from:origin,to:destination.id,type});
}
...
function refillLowerTiers(state,random){
  ...
  for(const team of state.teams.filter(t=>t.status==='active'&&t.tier==='u23'))while(team.roster.length<TIER_ROSTER.u23){const rider=makeRider(random,{teamId:team.id,tier:'u23',usedNames,ageRange:[18,19],year:state.year});state.riders.push(rider);team.roster.push(rider.id);}
