...
function makeRider(random,{teamId,tier,usedNames,ageRange=null,year=2026}){
  ...
  const terrain=pick(random,TERRAIN_TYPES),program=pick(random,PROGRAM_TYPES),profile=developmentProfile(random),debutAge=tier==='u23'?18:randInt(random,18,22),retirementAge=retirementAgeFor(random,rarity),careerLength=Math.max(10,retirementAge-debutAge),debutYear=year-(age-debutAge);
  const rider={
...
function makeTeam(random,row,tier){
  ...
  return {id,identity:TEAM_IDENTITIES[id]||`${name} Cycling`,name,nationality,tier,budget,attraction,sponsor:{name,investment:budget,attraction,expires:2026+sponsorDuration,started:2026},reputation:Math.round((budget+attraction)/2),facilities:clamp(Math.round((budget+attraction)/2)+randInt(random,-8,5),tier==='worldtour'?58:tier==='proseries'?45:28,96),salaryBudget:Math.round(budget*(tier==='worldtour'?1450000:tier==='proseries'?650000:170000)),expectation:tier==='worldtour'?'worldtour-points':tier==='proseries'?'promotion':'development',uciPoints:0,cyclePoints:0,roster:[],directorId:null,currentSeason:freshStats(),career:freshCareer(),history:[{year:2026,type:'sponsor',name}],status:'active',founded:randInt(random,1985,2020)};
}
...
function planCalendars(state){
  ...
  for(const rider of state.riders){
    ...
    const targetDays=rider.tier==='u23'?randInt(mulberry32(hashString(rider.id+state.year)),18,30):rider.role==='Domestique'?68:56;const targetCap=rider.program==='monuments'?16:rider.program==='stage-hunter'?15:rider.program==='one-week'?12:10;let days=0;
    for(const{event}of scored){if(days>=targetDays||rider.targetEvents.length>=targetCap)break;if(conflictsWithTargets(event,rider.targetEvents.map(id=>eventById(state,id)).filter(Boolean)))continue;rider.targetEvents.push(event.id);days+=event.stageProfiles.length;}
...
export function openNextSeason(state){
  upgradeUniverse(state);if(state.seasonStatus!=='complete')return{state,opened:false};const archive=state.pendingArchive||archiveSeason(state);if(!state.archives.some(a=>a.year===archive.year))state.archives.push(archive);state.pendingArchive=null;state.year+=1;for(const rider of state.riders)if(!rider.retired)rider.age+=1;for(const director of state.directors)if(!director.retired)director.age+=1;applyRetirementsAndDevelopment(state);applyTeamLifecycle(state);applySponsors(state);applyFacilities(state);applyDirectorMarket(state);applyTransfers(state);evolveEvents(state);prepareSeason(state);state.news.unshift({year:state.year,phase:'preseason',category:'New season',importance:'lead',headline:`Le Grand Braquet presents the ${state.year} peloton`,body:'New sponsors, promoted prospects, director moves and revised calendars open another season.'});return{state,opened:true,archive};
}
...
function expectedSalary(rider,tier=rider.tier){const ability=currentAbility(rider),rarityPremium={generational:1.65,legend:1.35,epic:1.15,rare:1,uncommon:.8,common:.62}[rider.rarity]||1,level=tier==='worldtour'?1:tier==='proseries'?.58:.22;return Math.round(ability*ability*920*rarityPremium*level);}
...
function riderHappiness(state,rider){const team=teamById(state,rider.teamId);if(!team)return 25;const salaryRatio=(rider.salary||1)/Math.max(1,expectedSalary(rider,team.tier)),director=directorById(state,team.directorId),sporting=(team.currentSeason?.uciPoints||0)/(team.tier==='worldtour'?2400:900),qualityFit=team.facilities/100,leadership=rider.role==='Leader'||rider.role==='Classics leader'?8:0;return clamp(Math.round(42+Math.min(22,(salaryRatio-.7)*40)+qualityFit*18+Math.min(14,sporting*7)+(director?director.ability/12:0)+leadership-(currentAbility(rider)>88&&team.tier!=='worldtour'?18:0)),10,98);}
function teamNeedScore(state,team){const points=team.currentSeason?.uciPoints||0,gt=team.currentSeason?.grandTours||0,monuments=team.currentSeason?.monuments||0,expected=team.tier==='worldtour'?Math.max(900,team.budget*23):Math.max(300,team.budget*9),failure=Math.max(0,(expected-points)/expected);return failure*45+(team.budget>=85&&gt===0?22:0)+(team.budget>=78&&gt===0&&monuments===0?10:0);}
function candidateScoreForTeam(state,rider,team,random){const ability=currentAbility(rider),potential=rider.potential,eliteFit=team.tier==='worldtour'&&potential>=90?18:team.tier==='proseries'&&potential>=90?-10:0,profileNeed=team.roster.map(id=>riderById(state,id)).filter(Boolean).filter(r=>r.terrain===rider.terrain).length<3?6:0,tierPenalty=team.tier==='worldtour'&&ability<72?-20:team.tier==='proseries'&&ability>91?-8:0,director=directorById(state,team.directorId),affordability=teamPayroll(state,team)+expectedSalary(rider,team.tier)<=team.salaryBudget?8:-18;return ability*.62+potential*.28+team.attraction*.13+team.facilities*.12+(director?.recruitment||60)*.08+profileNeed+eliteFit+tierPenalty+affordability+teamNeedScore(state,team)*.12+random()*7;}
function moveRider(state,rider,destination,year,type='transfer',fromId=null){
  ...
  rider.teamId=destination.id;rider.tier=destination.tier;rider.marketFrom=null;const contractRandom=mulberry32(hashString(`${rider.id}|${year}|contract`));rider.contractYears=randInt(contractRandom,2,4);rider.salary=Math.round(expectedSalary(rider,destination.tier)*(.88+contractRandom()*.28));rider.morale=clamp((rider.morale||65)+8,30,95);
...
function applyTransfers(state){
  ...
  const market=[],poachable=[];
  for(const rider of state.riders.filter(r=>!r.retired&&['worldtour','proseries'].includes(r.tier))){
    rider.contractYears=(rider.contractYears||1)-1;rider.happiness=riderHappiness(state,rider);
    const old=teamById(state,rider.teamId),underpaid=(rider.salary||0)<expectedSalary(rider,old?.tier)*.78,unhappy=rider.happiness<48,elite=['generational','legend','epic'].includes(rider.rarity);
    if(rider.contractYears<=0||unhappy||(underpaid&&elite&&random()<.55)){rider.marketFrom=old?.id||null;if(old)old.roster=old.roster.filter(id=>id!==rider.id);rider.teamId=null;market.push(rider);}else if(elite&&rider.happiness<66)poachable.push(rider);
  }
  const signed=new Set();
  const available=()=>[...market.filter(r=>!r.retired&&!signed.has(r.id)&&!r.teamId),...state.riders.filter(r=>!r.retired&&!signed.has(r.id)&&r.tier==='u23'&&r.age>=21),...state.riders.filter(r=>!r.retired&&!signed.has(r.id)&&r.tier==='continental')];
  const ordered=[...proTeams].sort((a,b)=>(teamNeedScore(state,b)+b.budget*.8)-(teamNeedScore(state,a)+a.budget*.8));
  for(const team of ordered){
    const targetSize=TIER_ROSTER[team.tier],aggression=teamNeedScore(state,team)+Math.max(0,team.budget-70);
    // Ambitious teams can buy out one unhappy elite rider before filling vacancies.
    if(team.tier==='worldtour'&&aggression>24&&team.roster.length>=targetSize&&random()<.65){const candidates=poachable.filter(r=>r.teamId&&r.teamId!==team.id&&!signed.has(r.id)&&teamPayroll(state,team)+expectedSalary(r,team.tier)<=team.salaryBudget).map(r=>({r,score:candidateScoreForTeam(state,r,team,random)+currentAbility(r)*.25})).sort((a,b)=>b.score-a.score);const choice=candidates[0]?.r;if(choice){const weakest=team.roster.map(id=>riderById(state,id)).filter(Boolean).sort((a,b)=>currentAbility(a)-currentAbility(b))[0];if(weakest&&currentAbility(choice)>currentAbility(weakest)+3){releaseTeamRider(state,team,weakest,`Released as ${team.name} rebuilt after an underperforming season.`);moveRider(state,choice,team,state.year,'poached',choice.teamId);signed.add(choice.id);}}}
    while(team.roster.length<targetSize){const candidates=available().filter(r=>team.tier!=='worldtour'||r.potential>=78||currentAbility(r)>=76).filter(r=>teamPayroll(state,team)+expectedSalary(r,team.tier)<=team.salaryBudget*1.08).map(r=>({r,score:candidateScoreForTeam(state,r,team,random)})).sort((a,b)=>b.score-a.score);let choice=candidates[0]?.r;if(!choice){const used=new Set(state.riders.map(r=>r.name));choice=makeRider(random,{teamId:null,tier:team.tier,usedNames:used,ageRange:[21,24],year:state.year});choice.marketFrom='new-pro';state.riders.push(choice);}const origin=choice.teamId||choice.marketFrom||'development';signed.add(choice.id);moveRider(state,choice,team,state.year,choice.tier==='u23'?'promotion':'transfer',origin);}
  }
...
function applyFacilities(state){const random=mulberry32(hashString(`${state.seed}|${state.year}|facilities`));for(const team of state.teams.filter(t=>t.status==='active'&&['worldtour','proseries'].includes(t.tier))){team.facilities=clamp((team.facilities||55)-randInt(random,0,2),30,98);const need=teamNeedScore(state,team),room=Math.max(0,(team.budget||50)-55);if((team.facilities<72||need>24)&&room>5){const investment=Math.min(randInt(random,2,7),Math.max(1,Math.floor(room/5)));team.facilities=clamp(team.facilities+investment,30,98);team.history.push({year:state.year,type:'facilities',value:team.facilities});}team.salaryBudget=Math.round(team.budget*(team.tier==='worldtour'?1450000:650000)*(0.88+team.facilities/500));}}
...
function applyDirectorMarket(state){
  ...
  const tierRank={worldtour:4,proseries:3,u23:2,continental:1};const teams=[...state.teams.filter(t=>t.status==='active')].sort((a,b)=>(teamNeedScore(state,b)+b.budget*.9+(tierRank[b.tier]||0)*10)-(teamNeedScore(state,a)+a.budget*.9+(tierRank[a.tier]||0)*10));
  for(const team of teams){const current=directorById(state,team.directorId),failure=teamNeedScore(state,team),fire=current&&(failure>28||((team.budget||0)>=82&&(team.currentSeason?.grandTours||0)===0&&random()<.62))&&random()<clamp(.22+failure/100,.22,.78);const pool=state.directors.filter(d=>!d.retired&&d.id!==current?.id&&(d.agencyId||d.teamId)).map(d=>{const origin=teamById(state,d.teamId),poachCost=origin&&tierRank[origin.tier]>=tierRank[team.tier]?-12:0,budgetPull=(team.budget-(origin?.budget||45))*.18;return{d,score:d.ability*.55+d.tactics*.17+d.recruitment*.15+d.development*.1+d.reputation*.08+budgetPull+poachCost+random()*4};}).sort((a,b)=>b.score-a.score),best=pool[0]?.d;if(!current&&best)appointDirector(state,best,team);else if(best&&(fire||(best.ability>current.ability+3&&team.budget>(teamById(state,best.teamId)?.budget||0)+8&&random()<.42)))appointDirector(state,best,team);}
  ...
  // Elite directors do not remain hidden in the lower tiers indefinitely: rich WorldTour teams actively poach them.
  const eliteDirectors=[...state.directors].filter(d=>!d.retired&&d.ability>=88).sort((a,b)=>b.ability-a.ability);
  for(const director of eliteDirectors){const origin=teamById(state,director.teamId);if(origin?.tier==='worldtour')continue;const destination=state.teams.filter(t=>t.status==='active'&&t.tier==='worldtour'&&t.budget>=(origin?.budget||35)+8).map(t=>({t,current:directorById(state,t.directorId)})).filter(x=>!x.current||x.current.ability<director.ability-2).sort((a,b)=>(b.t.budget+teamNeedScore(state,b.t))-(a.t.budget+teamNeedScore(state,a.t)))[0]?.t;if(destination)appointDirector(state,director,destination);}
